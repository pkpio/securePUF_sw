...
uint32_t isCalibrate = 0x00000011; //0x00000000 - calibration mode; 0xffffffff - normal execution mode ; 0x00000001 - temperature mode
uint32_t challenge = 0x00000000; //Challenge to PUF in calibration mode; In normal mode this is generated on board.
	
uint32_t numOpsWrite = 16;	//write Length;
uint32_t numOpsRead = 550;	//read Length;
uint32_t calib_numOpsRead = 2; //read length in calib mode
...
************************************************* All required changes to be done here  ************************************************************/ 
    ...
    LogIt(LOGIT_TIME_MARKER);
	start = GetTickCount();
	//Set parameter register 1 to the challenge (used, if in calibration)
	if(!SIRC_P->sendParamRegisterWrite(0, challenge)){
		tempStream << "Parameter register write failed with code " << (int) SIRC_P->getLastError();
		error(tempStream.str());
	}
	//Set parameter register 0 to the operand isCalibrate
	if(!SIRC_P->sendParamRegisterWrite(1, isCalibrate)){
		tempStream << "Parameter register write failed with code " << (int) SIRC_P->getLastError();
		error(tempStream.str());
	}
...
//############################################## LOOPING STARTS HERE ###########################################
		...
		for(int i=0; i<runSize; i++){
			...
			// Adjust counters
			for(int i = 1; i < (calib_numOpsRead+1); i++){
				if(i==1)
	...
	//////////////////// Normal test mode.
		...
		//Read the data back
		//cout<<"Atempting to read back responses"<<endl;
		if(!SIRC_P->sendRead(0, numOpsRead, outputValues)){
			tempStream << "Read from FPGA failed with code " << (int) SIRC_P->getLastError();
			error(tempStream.str());
		} else{
			//cout<<"Read back from memory success !"<<endl<<endl;
		...
		for(int i = 0; i < numOpsRead; i++){
			fprintf(fp, "%02x", outputValues[i]);
			if (choice == 2) { 
				if (bb) fprintf(fp, "\n");
				bb = !bb;
			}
